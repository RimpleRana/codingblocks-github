...
$('.promotionalbanner .closepromo').click(function(){
  ...
  // Reference the tab links.
	...
	tabLinks.click(function(event) {
    		var $this = $(this);		
    // Prevent event on live-courses tab
    if($this.hasClass('notInclude')) return; 
...
// Script for transition in vertical lines in Benefits section
// Example : "Benefits of our Placement Bootcamp for Working Professionals" in placement-bootcamp.html

document.addEventListener('DOMContentLoaded', () => {
  const verticleLineElements = document.querySelectorAll('.vertical__line');
  ...
  const handleScroll = (elements) => {
    elements.forEach(element => {
      const elementParentRect = element.parentElement.getBoundingClientRect();
      let initialPoint = elementParentRect.top;
      let finalPoint = window.innerHeight / 2 - elementParentRect.height;
      let elementHeight = finalPoint - initialPoint + elementParentRect.height;
      if (isElementInViewport(element)) {
        element.style.height = `${elementHeight}px`

        if (finalPoint >= initialPoint) {
          element.style.height = `${elementParentRect.height}px`;
        }
  ...
  // Initial check
  handleScroll(verticleLineElements);
...
// code  for timer in home section
...
const countDownDate = new Date("2025-03-30T23:59:59").getTime();
    ...
    const x = setInterval(function () {
      ...
      if (distance < 0) {
        ...
        body.classList.remove('haveCouponCard');

        return; 
      }

      couponCard.style.display = 'block';
      body.classList.add('haveCouponCard');
